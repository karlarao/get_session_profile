...
set heading off
set timing off
spool tmp_start_stack_10046_$DATE.$filter.sql
select txt from 
(
SELECT 'ORADEBUG SETORAPID ' || pid txt, 1 stat_order FROM v\$process WHERE addr = (SELECT paddr FROM v\$session WHERE sid = &&sid)
union all 
SELECT 'ORADEBUG TRACEFILE_NAME' txt, 2 stat_order from dual
union all
SELECT 'ORADEBUG EVENT 10046 TRACE NAME CONTEXT FOREVER, LEVEL 12' txt, 3 stat_order from dual
)
order by stat_order asc;
spool off

spool get_session_profile_$DATE.$filter.log append
@tmp_start_stack_10046_$DATE.$filter.sql
exit    
!
...
--snapper 
spool get_session_profile_$DATE.$filter.log append
@krlsnapper all,gather=a 5 1 &&sid
@krlsnapper all,gather=a 5 1 &&sid
@krlsnapper all,gather=a 5 1 &&sid
@krlsnapper all,gather=a 5 1 &&sid
@krlsnapper all,gather=a 5 1 &&sid
...
set lines 1000
@krl_ash_wait_chains.sql inst_id||'>achains>'||session_id||'>>'||program||'>>'||event2||'>>'||sql_id||'>>'||sql_opname||'>>'||top_level_call_name||'>>'||p1text||'>>'||p1||'>>'||p2text||'>>'||p2||'>>'||p3text||'>>'||p3||'>>'||blocking_session 1=1 sysdate-10/1440 sysdate

exit    
!

# execute short_Stack, end 10046, dump ash and wait chains
sqlplus -s /NOLOG <<! &
connect / as sysdba
--set timing off
define sid=$1
...
set heading off
set timing off
spool tmp_end_stack_10046_$DATE.$filter.sql
SELECT 'ORADEBUG SETORAPID ' || pid txt, 1 stat_order FROM v\$process WHERE addr = (SELECT paddr FROM v\$session WHERE sid = &&sid);
SELECT 'ORADEBUG TRACEFILE_NAME' txt from dual;
SELECT 'ORADEBUG SHORT_STACK' txt from dual;
SELECT 'exec dbms_lock.sleep(seconds => 5);' txt from dual;
SELECT 'ORADEBUG SHORT_STACK' txt from dual;
SELECT 'exec dbms_lock.sleep(seconds => 5);' txt from dual;
SELECT 'ORADEBUG SHORT_STACK' txt from dual;
SELECT 'exec dbms_lock.sleep(seconds => 5);' txt from dual;
...
SELECT 'ORADEBUG SETORAPID ' || pid txt, 1 stat_order FROM v\$process WHERE addr = (SELECT paddr FROM v\$session WHERE sid = &&sid)
union all 
SELECT 'ORADEBUG EVENT 10046 TRACE NAME CONTEXT OFF' txt, 2 stat_order from dual
union all
SELECT 'ORADEBUG FLUSH' txt, 3 stat_order from dual
union all
SELECT 'ORADEBUG CLOSE_TRACE' txt, 4 stat_order from dual
)
order by stat_order asc;
spool off

spool get_session_profile_shortstack_$DATE.$filter.log append
@tmp_end_stack_10046_$DATE.$filter.sql
spool off
...
"PGA_ALLOCATED , TEMP_SPACE_ALLOCATED " > myash_&&sid..$DATE.$filter.csv

spool myash_&&sid..$DATE.$filter.csv append
